...
function write_out {
    ...
    echo "COMMITTED_APPROVAL_ROSERED=$COMMITTED_APPROVAL_ACME"         >> $CC2_ENV_FOLDER/get-cc-info
    echo "COMMITTED_APPROVAL_MARUKO=$COMMITTED_APPROVAL_BUDGET"         >> $CC2_ENV_FOLDER/get-cc-info
}
...
if [ "$?" == "0" ]; then
    ...
    COMMITTED_APPROVAL_ACME=$(echo $OUTPUT | jq .approvals.RoseredMSP)
    COMMITTED_APPROVAL_BUDGET=$(echo $OUTPUT | jq .approvals.MarukoMSP)
...
if [ "$ECHO_INFO" == "true" ]; then
    ...
    echo "COMMITTED_APPROVAL_ROSERED=$COMMITTED_APPROVAL_ACME"
    echo "COMMITTED_APPROVAL_MARUKO=$COMMITTED_APPROVAL_BUDGET"
fi
