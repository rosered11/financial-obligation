...
if [ "$FABRIC_CFG_PATH" == "" ]; then 
   ...
   echo "> source  set-env.sh  rosered"
   echo "> source  set-env.sh  maruko"
   exit
fi
...
function update_properties {
    ...
    case  "$CC2_ENDORSING_PEERS" in
        "auto")
            ...
            ;;
        "rosered")
            CC2_ENDORSING_PEERS="--peerAddresses=rosered-peer1.rosered.com:7051"
            ;;
        "maruko")
            CC2_ENDORSING_PEERS="--peerAddresses=maruko-peer1.maruko.com:8051"
            ;;
        "both")
            CC2_ENDORSING_PEERS="--peerAddresses=rosered-peer1.rosered.com:7051 --peerAddresses=maruko-peer1.maruko.com:8051"
            ;;
        *)
            CC2_ENDORSING_PEERS="--peerAddresses=$CORE_PEER_ADDRESS"
    esac
